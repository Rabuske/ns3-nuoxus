...
int main (int argc, char *argv[])
{
    ...
    uint64_t diskSizeKb         = 40000000; //10 gb
    TypeId   cachingStrategy    = NuoxusCachingStrategyDefault::GetTypeId();
    char     cachingStrategyIndicator = 'D';
    ...
    cmd.AddValue("cachingStrategy", "Type of caching strategy", cachingStrategyIndicator);
    ...
    switch(cachingStrategyIndicator){
	case 'R':
	    cachingStrategy = NuoxusCachingStrategyLRU::GetTypeId();
            break;
        case 'N':
            cachingStrategy = NuoxusCachingStrategyNoCache::GetTypeId();
            break;
        case 'F':
            cachingStrategy = NuoxusCachingStrategyLFU::GetTypeId();
            break;
        default:
            cachingStrategy = NuoxusCachingStrategyDefault::GetTypeId();
            break;
    }
